...
namespace DoAnLapTrinhWeb.Models
{
    public class MoMoSecurity
    {
        ...
        public MoMoSecurity()
        {
            //Mã hóa và giải mã mật khẩu bằng cách sử dụng bảo mật
        }
            ...
            string amount, string paymentCode, string storeId, string storeName, string publicKeyXML)
        {
            ...
            using (var rsa = new RSACryptoServiceProvider(4096))
            {
                try
                {
                    //Khóa công khai của MoMo có định dạng PEM.
        ...
        public string buildQueryHash(string partnerCode, string merchantRefId, string requestid, string publicKey)
        {
            ...
            using (var rsa = new RSACryptoServiceProvider(2048))
            {
                try
                {
                    //Máy khách mã hóa dữ liệu bằng khóa công khai do máy chủ cấp
